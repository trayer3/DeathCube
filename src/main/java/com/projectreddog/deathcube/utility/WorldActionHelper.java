...
public class WorldActionHelper {
	...
	public static boolean doFakeExpolsion(World world, BlockPos bp, float size, DamageSource damageSource, boolean RemoveBlockAtLocation) {
		...
		if (RemoveBlockAtLocation && !world.isRemote) {
			world.setBlockToAir(bp);
		}

		// particles
		if (size >= 2.0F) {
			world.spawnParticle(EnumParticleTypes.EXPLOSION_HUGE, explosionX, explosionY + 1, explosionZ, 1.0D, 0.0D, 0.0D, new int[0]);
		} else {
			world.spawnParticle(EnumParticleTypes.EXPLOSION_LARGE, explosionX, explosionY + 1, explosionZ, 1.0D, 0.0D, 0.0D, new int[0]);
			world.spawnParticle(EnumParticleTypes.FIREWORKS_SPARK, explosionX, explosionY + 1, explosionZ, 1.0D, 0.0D, 0.0D, new int[0]);
		}

		world.spawnParticle(EnumParticleTypes.EXPLOSION_NORMAL, explosionX, explosionY + 1, explosionZ, 0, 0, 0, new int[0]);
		world.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, explosionX, explosionY + 1, explosionZ, 0, 0, 0, new int[0]);
